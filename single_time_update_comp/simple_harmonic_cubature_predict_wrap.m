function [xS_predict] = simple_harmonic_cubature_predict_wrap(continuous_discrete_model,timesteps,xS0)
...
nPts = 2 * nx;
sig1 = sqrt(continuous_discrete_model.K_process(1,1));%sigma1 in process noise
sig2 = sqrt(continuous_discrete_model.K_process(3,3));%sigma2 in process noise
c23 = sqrt(continuous_discrete_model.K_process(2,3));
...
sigma_point_count = size(Xi,2);
    ...
    x = Xi(:,i);
        f = [x(2), x(3), -x(1)]';
        L0 = [x(3);-x(1);-x(2)];%d/dt, f_i d/dx_i == 0, second deriv 0
