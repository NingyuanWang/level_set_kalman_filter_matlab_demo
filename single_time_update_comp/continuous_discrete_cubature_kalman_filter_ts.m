function [varargout] = continuous_discrete_cubature_kalman_filter_ts(xS_guess,measurements,continuous_discrete_model,instance_parameter)
...
%	instance_parameter: of instance_parameter_cls type
    ...
    for n = 1:instance_parameter.observation_count
        %Prediction part
        if nargout <=2
            for j = 1:instance_parameter.ode_subdivision
                dtc = dt / instance_parameter.ode_subdivision;
                ts = t + (j-1)*dtc;
                te = t + j*dtc;
                [xS] = simple_harmonic_cubature_predict_wrap(continuous_discrete_model,[ts,te],xS);
            end
            %WARNING: Contrary to the level set version, the implementation
            %is specfic to the test problen. 
        else
            for j = 1:instance_parameter.ode_subdivision
                ts = t + (j-1)*dt;
                te = t + j*dt;
                [xS,t_int,X_int] = simple_harmonic_cubature_predict_wrap(continuous_discrete_model,[ts,te],xS);
            end
