...
%	instance_parameter: of instance_parameter_cls type
    ...
    for n = 1:instance_parameter.observation_count
        %Prediction part
        if nargout <=2
            [xS] = simple_harmonic_cubature_predict_wrap(continuous_discrete_model,linspace(t,t+dt,instance_parameter.ode_subdivision+1),xS);
            %WARNING: Contrary to the level set version, the implementation
            %is specfic to the test problen. 
        else
            [xS,t_int,X_int] = simple_harmonic_cubature_predict_wrap(continuous_discrete_model,linspace(t,t+dt,instance_parameter.ode_subdivision+1),xS);
