...
for j = 1:numel(w0_degree_list)
    ...
    plot(log2(m_list),RMSE_vm_CDCKF(7,:)*180/pi,'-o')
    hold on
    plot(log2(m_list),RMSE_vm_LSKF_fixed(7,:)*180/pi,'--x')
    plot(log2(m_list),RMSE_vm_LSKF_adaptive(7,:)*180/pi,'-.+')
    axis([0 7 0.15 0.25])
    %legend('CDCKF','LSKF-fixed RK4','LSKF-adaptive')
...
exportgraphics(t,'RMSE_2_sec_n.png','Resolution',450)
savefig('RMSE_2_sec_n.fig')
